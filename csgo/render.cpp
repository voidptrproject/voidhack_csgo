...
void render::internal::render_hook(IDirect3DDevice9* dev, const memory::address_t& return_address) {
	static std::once_flag once_flag; std::call_once(once_flag, [&]() {
		...
		if (auto hwnd = FindWindow("Valve001", 0); hwnd) {
			ImGui_ImplWin32_Init(hwnd);
			ImGui_ImplDX9_Init(dev);

			static char* imguifilename = new char[MAX_PATH];
			strcpy(imguifilename, (env::get_data_path() / "gui.dat").make_preferred().generic_string().c_str());
			ImGui::GetIO().IniFilename = imguifilename;
